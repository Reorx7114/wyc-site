...
async function supabaseRequest<T>(path: string, init?: RequestInit): Promise<SupabaseResult<T>> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: `Supabase request failed (${response.status} ${response.statusText}) at ${endpoint}: ${message}`,
...
export async function getContentItems(type: ContentType): Promise<ContentItem[]> {
  const rows = await supabaseFetch<Array<SupabaseBlogRow | SupabaseEventRow>>(
...
export async function upsertContentItem(type: ContentType, item: ContentItem) {
  ...
  return supabaseRequest<Array<SupabaseBlogRow | SupabaseEventRow>>(
